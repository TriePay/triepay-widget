...
export default class Web3Wallet {
  ...
  registerCallback() {
    this.callback = {};
    this.callback['account'] = this.on('account', (account) => {
        if (this.onStateChangedCallback) {
          this.onStateChangedCallback('account', account);
        }
    });

    this.callback['network'] = this.on('network', (chainId) => {
      if (this.onStateChangedCallback) {
        this.onStateChangedCallback('network', chainId);
      }
    });

    this.callback['disconnect'] = this.on('disconnect', () => {
      if (this.onStateChangedCallback) {
        this.onStateChangedCallback('disconnect');
      }
    });

    this.callback['connect'] = this.on('connect', (connectInfo) => {
      if (this.onStateChangedCallback) {
        //connectInfo {chainId}
        this.onStateChangedCallback('connect', connectInfo);
      }
    });
  }
  ...
  on(event, callback) {
    ...
    switch (event) {
      ...
      case 'accounts':
        ...
        break
      case 'network':
        internalCallback = (chainId) => callback(chainId)
      ...
      case 'disconnect':
        ...
        break
      case 'connect':
        internalCallback = (connectInfo) => callback(connectInfo)
        window.ethereum.on('connect', internalCallback)
        break;
    }
  ...
  off(event, internalCallback) {
    switch (event) {
      ...
      case 'disconnect':
        ...
        break
      case 'connect':
        window.ethereum.removeListener('connect', internalCallback)
        break;
    }
    return internalCallback
  }
  ...
  sendAsync (request, callback) {
    window.ethereum.request(request)
        .then(response => { callback(undefined, makeJsonRpcResponse(request, response)) })
        .catch(e => { callback(e, makeJsonRpcResponse(request, undefined, e)) });
  }
}
